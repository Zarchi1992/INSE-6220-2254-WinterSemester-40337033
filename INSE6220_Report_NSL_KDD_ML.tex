...
Fig.~\ref{fig:3} illustrates the learning curves for the selected models after applying PCA, including Extra Trees (ET), Random Forest (RF), and K-Nearest Neighbour (KNN). The plots show the relationship between the number of training instances and model performance, where the blue line represents the training score and the green line represents the cross-validation score. It can be observed that both ET and RF achieve near-perfect training accuracy, close to 1.0, indicating strong fitting to the training data. Their cross-validation scores gradually improve as the training size increases and stabilize around 0.993–0.994, showing consistent generalization performance. KNN, on the other hand, exhibits slightly lower training accuracy compared to ET and RF, but its cross-validation score steadily increases and reaches comparable performance as more data is introduced. The gap between the training and validation curves is relatively small for all models, indicating that overfitting is minimal and the models generalize well to unseen data. Additionally, the smooth convergence of the curves suggests that increasing the training data contributes to improved performance and model stability. Overall, ET and RF demonstrate more stable and consistent learning behavior, while KNN shows gradual improvement with increased data, reflecting its sensitivity to data distribution.
